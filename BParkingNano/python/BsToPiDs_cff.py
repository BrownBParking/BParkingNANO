...
#        ' pt()> 3.0 '
        '( (mass() > 1.92 && mass() < 2.0) )'
#        '( (mass() > 1.66 && mass() < 2.26) )'
#        '&& userFloat("leadTrkPt") > 1.5'
        ),
        postVtxSelection = cms.string(
        'userFloat("sv_prob") > 1.e-2'
        '&&  (  (userFloat("fitted_mass") > 1.92 && userFloat("fitted_mass") < 2.0)  )'
#        '&&  (  (userFloat("fitted_mass") > 1.66 && userFloat("fitted_mass") < 2.26)  )'
        '&& userFloat("l_xy")/userFloat("l_xy_unc") > 5. '
)
)

BsToPiDs = cms.EDProducer(
    'BsToPiDsBuilder',
    Ds = cms.InputTag('DsToKKPi'),
    DsTransientTracks = cms.InputTag('tracksBPark', 'SelectedTransientTracks'),
    ...
    piSelection = cms.string('pt > 1.0 && abs(eta)<2.4'),
    isoTracksSelection = cms.string('pt > 0.5 && abs(eta)<2.5'),
    preVtxSelection = cms.string(
#        'pt > 5.'# && userFloat("min_dr") > 0.4 '
        ' ( (mass > 4.0 && mass < 7.0) )'
#        ' ((mass > 5.23 && mass < 5.3) | (mass > 5.44 && mass < 5.51))'
        ),
    postVtxSelection = cms.string(
...
#        '&& ((userFloat("fitted_mass") > 5.23 && userFloat("fitted_mass") < 5.3) | (userFloat("fitted_mass") > 5.44 && userFloat("fitted_mass") < 5.51))'
...
DsToKKPiTable = cms.EDProducer(
   'SimpleCompositeCandidateFlatTableProducer',
    src = cms.InputTag("DsToKKPi"),
    cut = cms.string(""),
    name = cms.string("Ds"),
...
BsToPiDsTable = cms.EDProducer(
    'SimpleCompositeCandidateFlatTableProducer',
    src = cms.InputTag("BsToPiDs"),
    cut = cms.string(""),
    name = cms.string("BsToPiDs"),
    doc = cms.string("BsToPiDs Variable"),
    singleton=cms.bool(False),
    extension=cms.bool(False),
    variables=cms.PSet(
        # pre-fit quantities
        ...
        fit_barD0_mass = ufloat('barMassDs_fullfit'),
...
CountBsToPiDs = cms.EDFilter("PATCandViewCountFilter",
