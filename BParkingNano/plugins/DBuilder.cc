...
class DBuilder : public edm::global::EDProducer<> {
...
public:
  ...
  explicit DBuilder(const edm::ParameterSet &cfg):
...
void DBuilder::produce(edm::StreamID, edm::Event &evt, edm::EventSetup const &) const {

  //std::cout<<"beginning of D selection"<<std::endl;
  ...
  for(size_t trk1_idx = 0; trk1_idx < pfcands->size(); ++trk1_idx ){
    ...
    for(size_t trk2_idx = 0; trk2_idx < pfcands->size(); ++trk2_idx) {
     ...
     for(size_t trk3_idx = trk2_idx+1; trk3_idx < pfcands->size(); ++trk3_idx) {
      ...
      // create a D candidate; add first quantities that can be used for pre fit selection
      ...
      D_cand.setP4(trk1_p4 + trk2_p4 + trk3_p4);
      D_cand.addUserFloat("trk_deltaR", reco::deltaR(*trk1_ptr, *trk2_ptr));

      // save indices
      D_cand.addUserInt("trk1_idx", trk1_idx );
      D_cand.addUserInt("trk2_idx", trk2_idx );
      D_cand.addUserInt("trk3_idx", trk3_idx );

      // save cands      
      D_cand.addUserCand("trk1", trk1_ptr );
      D_cand.addUserCand("trk2", trk2_ptr );
      D_cand.addUserCand("trk3", trk3_ptr );
      ...
      D_cand.addUserFloat("barMass", (trk1_p4 + trk2_p4 + trk3_p4).M() );
      ...
      // std::cout<<fitter.success()<<std::endl;
       ...
       D_cand.addUserFloat("sv_ndof", fitter.dof()); 
       D_cand.addUserFloat("sv_prob", fitter.prob());    
       D_cand.addUserFloat("fitted_mass", fitter.fitted_candidate().mass() );
       D_cand.addUserFloat("fitted_pt", fitter.fitted_candidate().globalMomentum().perp() );
       D_cand.addUserFloat("fitted_eta", fitter.fitted_candidate().globalMomentum().eta() );
       D_cand.addUserFloat("fitted_phi", fitter.fitted_candidate().globalMomentum().phi() );
